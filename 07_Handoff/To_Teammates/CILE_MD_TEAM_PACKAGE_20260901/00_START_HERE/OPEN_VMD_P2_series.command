...
SOURCE_PDB="$SOURCE_DIR/03_P2_series_50ps_animation.pdb"
...
if [[ ! -f "$SOURCE_PDB" || ! -f "$SOURCE_SCRIPT" ]]; then
  echo "P2 계열 VMD 파일을 찾을 수 없습니다."
  echo "$SOURCE_PDB"
  echo "$SOURCE_SCRIPT"
  echo
  echo "닫으려면 Enter를 누르세요."
...
if [[ "$MODEL_COUNT" -eq 0 || "$MODEL_COUNT" -ne "$ENDMDL_COUNT" ]]; then
  echo "P2 계열 PDB 프레임 구조가 올바르지 않습니다."
